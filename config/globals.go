...
func attemptEvaluateGlobals(
	...
	for _, global := range globals {
		if canEvaluateGlobal(terragruntOptions, global.Expr, evaluatedGlobals) {
...
// canEvaluateGlobal determines if the global expression can be evaluated. An expression can be evaluated if one of the
// following is true:
// - It has no references to other globals.
// - It has references to other globals that have already been evaluated.
func canEvaluateGlobal(
	terragruntOptions *options.TerragruntOptions,
	...
	if len(vars) == 0 {
		// If there are no global variable references, we can evaluate this expression.
		return true
	}
	...
	for _, var_ := range vars {
		...
		// We can't evaluate any variable other than `global` here.
		if var_.RootName() != "global" {
			return false
		}

		// If we can't get any global name, we can't evaluate it.
		globalName := getGlobalName(terragruntOptions, var_)
		if globalName == "" {
			return false
		}

		// If the referenced global isn't evaluated, we can't evaluate this expression.
		_, hasEvaluated := evaluatedGlobals[globalName]
		if !hasEvaluated {
			return false
		}
	}
